...
<main class="flex-grow w-full max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8">
   ...
   <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col gap-1 group">
            ...
            <span class="text-3xl font-bold text-[#111418]">{{ number_format($stats['total_jemaat']) }}</span>
        ...
        <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col gap-1 group">
            <div class="flex items-center justify-between mb-2">
                <span class="text-sm font-medium text-gray-500">Jadwal Mendatang</span>
                <div class="p-2 bg-purple-50 text-purple-600 rounded-lg group-hover:bg-purple-100 transition-colors">
                    <span class="material-symbols-outlined">event</span>
                </div>
            </div>
            <span class="text-3xl font-bold text-[#111418]">{{ $stats['jadwal_mendatang'] }}</span>
            <span class="text-xs text-gray-500 font-medium">Ibadah mendatang</span>
        </div>

        <!-- Stat Card 3 -->
        <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col gap-1 group">
            <div class="flex items-center justify-between mb-2">
                <span class="text-sm font-medium text-gray-500">Berita Terbaru</span>
                <div class="p-2 bg-orange-50 text-orange-600 rounded-lg group-hover:bg-orange-100 transition-colors">
                    <span class="material-symbols-outlined">newspaper</span>
                </div>
            </div>
            <span class="text-3xl font-bold text-[#111418]">{{ $stats['berita_terbaru'] }}</span>
            <span class="text-xs text-secondary font-medium flex items-center gap-1">
        ...
        <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col gap-1 group">
            <div class="flex items-center justify-between mb-2">
                <span class="text-sm font-medium text-gray-500">Total Saran</span>
                <div class="p-2 bg-green-50 text-secondary rounded-lg group-hover:bg-green-100 transition-colors">
    ...
    <section class="space-y-6">
        <div class="flex items-center justify-between">
            ...
            <a class="text-sm font-bold text-primary hover:text-primary-dark flex items-center gap-1" href="{{ route('jadwal.index') }}">
            ...
            <div class="bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-all group">
                ...
                <div class="p-5 flex flex-col gap-4">
                    <div class="flex justify-between items-start">
                        <span class="px-2.5 py-1 rounded-md {{ $index == 0 ? 'bg-primary/10 text-primary' : ($index == 1 ? 'bg-secondary/10 text-secondary' : 'bg-blue-100 text-blue-700') }} text-xs font-bold uppercase tracking-wide">
                            Ibadah
                        </span>
    ...
    <section class="space-y-6 pb-8">
        <div class="flex items-center justify-between">
            ...
            <a class="text-sm font-bold text-primary hover:text-primary-dark flex items-center gap-1" href="{{ route('berita.index') }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <article class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all flex flex-col h-full border border-gray-100">
                <div class="relative h-48 overflow-hidden group">
                    @if($berita->gambar)
                        <img alt="{{ $berita->judul }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" src="{{ asset('storage/' . $berita->gambar) }}">
                    @else
                ...
                <div class="p-5 flex flex-col flex-1">
                    ...
                    <a class="text-sm font-bold text-primary hover:text-primary-dark inline-flex items-center gap-1 mt-auto" href="{{ route('berita.show', $berita->id) }}">
